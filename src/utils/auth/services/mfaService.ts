...
/**
 * Stores the MultiFactorResolver from a signInWithEmailAndPassword rejection
 * (auth/multi-factor-auth-required) so the MFA challenge page can pick it up.
 */
export async function storeMfaResolver(error: unknown, email: string): Promise<boolean> {
